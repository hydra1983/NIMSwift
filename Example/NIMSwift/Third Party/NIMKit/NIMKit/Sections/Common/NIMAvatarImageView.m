...
- (void)setAvatarBySession:(NIMSession *)session
{
    ...
    if (info.avatarUrlString != nil ){
         NSString *urlS =  [self trimLast: info.avatarUrlString];
...
/**
 删除？以后的字符（云信添加的）
 **/
- (NSString *)trimLast:(NSString *) url
{
    NSString *str = url;
    if (str != nil) {
        NSRange last = [str rangeOfString:@"?" options:NSBackwardsSearch];
        NSString *urlS = [str substringToIndex:last.location];
