...
// 拓展类型（0:群，1:团队）
@property(nonatomic,assign) NSInteger extType;
...
- (instancetype)initWithTeam:(NIMTeam *)team{
    ...
    if (self) {
        ...
        NSString *type = team.clientCustomInfo;
        if (type) {
            NSData *data = [type dataUsingEncoding:NSUTF8StringEncoding];
            NSDictionary *json = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingAllowFragments error:nil];
            _extType = [[json valueForKey:@"type"] integerValue];
        }
...
- (void)requestData:(void(^)(NSError *error)) handler{
    ...
    [[NIMSDK sharedSDK].teamManager fetchTeamMembers:self.team.teamId completion:^(NSError *error, NSArray *members) {
        if (!error) {
            ...
            [wself.view makeToast:_extType == 0?@"你已经不在群里":@"你已经不在团队" duration:2
...
- (void)buildBodyData{
    ...
    teamMember.title = _extType == 0?@"群成员":@"团队成员";
    ...
    teamName.title = _extType == 0?@"群名称":@"团队名称";
    ...
    teamNick.title = _extType == 0?@"群昵称":@"团队昵称";
    ...
    teamIntro.title = _extType == 0?@"群介绍":@"团队介绍";
    teamIntro.subTitle = self.team.intro.length ? self.team.intro : (canEdit ? _extType == 0?@"点击填写群介绍":@"点击填写团队介绍" : @"");
    ...
    teamAnnouncement.title = _extType == 0?@"群公告":@"团队公告";
    teamAnnouncement.subTitle = _extType == 0?@"点击查看群公告":@"点击查看团队公告";//self.team.announcement.length ? self.team.announcement : (isManager ? @"点击填写群公告" : @"");
    ...
    itemQuit.title = _extType == 0?@"退出高级群":@"退出团队";
    ...
    itemDismiss.title  = _extType == 0?@"解散群聊":@"解散团队";
    ...
    itemUpdateInfo.title = _extType == 0?@"群资料修改权限":@"团队资料修改权限";
...
- (void)onMore:(id)sender{
    _moreActionSheet = [[UIActionSheet alloc] initWithTitle:@"请操作" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:_extType == 0?@"转让群":@"转让团队",_extType == 0?@"转让群并退出":@"转让团队并退出",nil];
...
- (void)onTouchAvatar:(id)sender{
    ...
    {
        _avatarActionSheet = [[UIActionSheet alloc] initWithTitle:_extType == 0?@"设置群头像":@"设置团队头像" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"拍照",@"从相册", nil];
        [_avatarActionSheet showInView:self.view];
    }
}

- (void)updateTeamName{
    _updateTeamNameAlertView = [[UIAlertView alloc] initWithTitle:@"" message:_extType == 0?@"修改群名称":@"修改团队名称" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确认", nil];
    _updateTeamNameAlertView.alertViewStyle = UIAlertViewStylePlainTextInput;
    [_updateTeamNameAlertView show];
}

- (void)updateTeamNick{
    _updateTeamNickAlertView = [[UIAlertView alloc] initWithTitle:@"" message:_extType == 0?@"修改群昵称":@"修改团队昵称" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确认", nil];
    _updateTeamNickAlertView.alertViewStyle = UIAlertViewStylePlainTextInput;
    [_updateTeamNickAlertView show];
}

- (void)updateTeamIntro{
    _updateTeamIntroAlertView = [[UIAlertView alloc] initWithTitle:@"" message:_extType == 0?@"修改群介绍":@"修改团队介绍" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确认", nil];
...
- (void)quitTeam{
    _quitTeamAlertView = [[UIAlertView alloc] initWithTitle:@"" message:_extType == 0?@"确认退出群聊?":@"确认退出团队?" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确认", nil];
    [_quitTeamAlertView show];
}

- (void)dismissTeam{
    _dismissTeamAlertView = [[UIAlertView alloc] initWithTitle:@"" message:_extType == 0?@"确认解散群聊?":@"确认解散团队?" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确认", nil];
...
- (void)changeInviteMode{
    _inviteActionSheet = [[UIActionSheet alloc] initWithTitle:_extType == 0?@"邀请他人入群":@"邀请他人入团队" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"管理员邀请",@"所有人邀请", nil];
    [_inviteActionSheet showInView:self.view];
}

- (void)changeUpdateInfoMode{
    _updateInfoActionSheet = [[UIActionSheet alloc] initWithTitle:_extType == 0?@"群资料修改权限":@"团队资料修改权限" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"管理员修改",@"所有人修改", nil];
...
- (void)didFinishedSelect:(NSArray *)selectedContacts{
    ...
    NSString *postscript = [NSString stringWithFormat:@"邀请你加入: %@",self.team.teamName];
